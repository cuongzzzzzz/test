      ...
      return ctx.db.connection().execute(async (conn) =>
        ...
        conn
          ...
          .select([
            ...
            'totalFriendCount',
            (eb) =>
              mutualFriendCount(eb, ctx.session.userId, input.friendUserId).as('mutualFriendCount'),
          ])
...
const mutualFriendCount = (eb: any, userId: number, friendUserId: number) => {
  return eb.selectFrom('friendships as f1')
    .innerJoin('friendships as f2', (join: any) =>
      join
    ...
    .select((eb: any) => [
      eb.fn.count('f1.friendUserId').as('count'),
    ])
}
